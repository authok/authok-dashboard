...
import ApplicationSettings from '@/pages/connection/components/ApplicationSettings';
import { history } from 'umi';
import { formatPath } from '@/utils/link.utils';
import EmailConnectionSettings from './components/EmailProviderSettings';
import TrySendEmailModal from './components/TrySendEmailModal';

const EmailConnectionDetailsPage: React.FC<any> = ({ match }) => {
    ...
    <PageContainer className="page-middle">
            ...
            <Row justify="center" gutter={16}>
              ...
              <Col span={4}>
                <Row style={{ justifyItems:'end' }}>
                  <TrySendEmailModal connection={connection} trigger={<Button size="large" icon={<CaretRightOutlined/>}>测试</Button>}/>
                </Row>
              </Col>
            </Row>
          }
      ...
      <ProCard
        ...
        tabs={{
          ...
          activeKey: tab,
          onChange: (key) => {
            history.push(formatPath(`/connections/passwordless/email/${id}/${key}`));
          },
          tabBarGutter: 64,
          size: 'large',
      }}>
        <ProCard.TabPane key="settings" tab="设置">
          <EmailConnectionSettings connection={connection} onUpdate={handleUpdate} loadingUpdate={loadingUpdate} />
        </ProCard.TabPane>
...
export default EmailConnectionDetailsPage;